...
type Props = {
  featuredEstate: Estate;
};

const FeaturedEstate: FC<Props> = props => {
  const { featuredEstate } = props;
    ...
    <section className='flex md:flex-row flex-col px-4 py-10 items-center gap-12 container mx-auto'>
      <div className='md:grid gap-8 grid-cols-1'>
        <div className='rounded-2xl overflow-hidden h-48 mb-4 md:mb-0'>
          <Image
            src={featuredEstate.coverImage.url}
        ...
        <div className='grid grid-cols-2 gap-8 h-48'>
          {featuredEstate.images.splice(1, 2).map(image => (
      ...
      <div className='md:py-10 md:w-1/2 text-left'>
        <h3 className='font-heading mb-12'>Featured Estate</h3>

        <p className='font-normal max-w-md'>{featuredEstate.description}</p>
          ...
          <div className='flex mb-3 md:mb-0'>
            <div className='flex gap-3 flex-col items-center justify-center mr-4'>
              ...
              <p className='md:font-bold flex font-medium text-lg xl:text-5xl'>
                PHP {featuredEstate.price}
              </p>
            </div>
            <div className='flex gap-3 flex-col items-center justify-center mr-4'>
              <p className='text-xs lg:text-xl text-center'>Discount</p>
              <p className='md:font-bold flex font-medium text-lg xl:text-5xl'>
                PHP {featuredEstate.discount}
              </p>
            </div>
          </div>

          <Link
            href={`/Estates/${featuredEstate.slug.current}`}
